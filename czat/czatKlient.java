        ...
        try (Socket socket = new Socket(SERVER_ADDRESS, SERVER_PORT); // Nawiąż połączenie z serwerem
            ...
            while (true) {
                ...
                if (userName == null || userName.trim().isEmpty()) {
                    ...
                    out.println(userName); //wysywlamy login do serwera
                    break;
                }
            }
            ...
            new Thread(() -> {
                ...
                try {
                    while ((line = in.readLine()) != null) {
                        ...
                        else if (line.startsWith("PRYWATNA WIADOMOSC")) {
                        // Obsługa prywatnych wiadomości
                        System.out.println("[PRYWATNA] " + line.substring("PRYWATNA WIADOMOSC".length()).trim());
                        }
                        else {
...
//            }
            ...
            String userInput; // // Główna pętla wysyłająca wiadomości użytkownika do serwer
...
//1. W terminalu, przejdź do folderu, gdzie są te pliki, i skompiluj je
//javac ChatServer.java
/// ChatClient.java
//W tym samym folderze uruchom serwer:> java ChatServer
//3. Uruchamianie klientów
//
//W osobnym terminalu (lub nowej karcie/oknie): -> java ChatClient
//Możesz uruchamiać dowolną liczbę klientów, każdemu otwierając osobne okno terminala